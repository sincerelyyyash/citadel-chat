...
export const transcribeAudio = httpAction(async (ctx, request) => {
    try {
        ...
        if (!GROQ_API_KEY || GROQ_API_KEY === "your-groq-api-key-here") {
            console.error("GROQ_API_KEY not configured. Please set the environment variable.")
            return new Response(
                JSON.stringify({
                    error: "Voice input service not configured. Please contact the administrator to set up the Groq API key."
                }),
                {
                    status: 500,
                    headers: { "Content-Type": "application/json" }
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                ...
                return new Response(
                    JSON.stringify({
                        error: "Invalid API key. Please check the Groq API key configuration."
                    }),
                    {
                        status: 500,
                        headers: { "Content-Type": "application/json" }
