    ...
    for await (const message of messages) {
        ...
        if (message.role === "user") {
            ...
            for (const p of message.parts) {
                ...
                if (p.type === "file") {
                    const filename = p.filename || "attached-file"
                    const fileTypeInfo = getFileTypeInfo(filename, p.mimeType)
                    if (fileTypeInfo.isImage) failedFileFetch("image", filename)
                    else if (fileTypeInfo.isPdf) failedFileFetch("pdf", filename)
                    else failedFileFetch("text", filename)
                }
            }
            ...
            for (const p of message.parts) {
                if (p.type === "text") {
                    ...
                    mapped_content.push({
                        type: "text",
                        text: `<internal-system-error>Attachments are disabled in this deployment. File "${p.filename || "unknown"}" cannot be loaded.</internal-system-error>`
                    })
