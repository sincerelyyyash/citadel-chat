...
export const manualStreamTransform = (
    ...
    _userId: string,
    _actionCtx: GenericActionCtx<DataModel>
) => {
    ...
    return new TransformStream<TextStreamPart<any>, DataStreamString>({
        transform: async (chunk, controller) => {
            ...
            switch (chunkType) {
                ...
                case "file": {
                    // Attachments are disabled; keep stream valid and surface a helpful message.
                    controller.enqueue(
                        formatDataStreamPart(
                            "text",
                            "\n\n[Attachment output omitted: storage disabled]\n"
                        )
                    )
                    break
                }
