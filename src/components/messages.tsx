import { CharacterMessageAvatar } from "@/components/character-message-avatar"
import type { useChatIntegration } from "@/hooks/use-chat-integration"
import { browserEnv } from "@/lib/browser-env"
import { useChatStore } from "@/lib/chat-store"
import { asMessageWithMetadata } from "@/lib/chat-message-metadata"
...
import { AlertCircle, Code, FileType, FileType2, Image as ImageIcon, RotateCcw } from "lucide-react"
...
const getFileIcon = (part: { data: string; filename?: string; mimeType?: string }) => {
    ...
    if (isImage) return <ImageIcon className="size-4 text-chart-1" />
...
const getMessageCharacterIdForAvatar = (
    message: UIMessage,
    lastMessage: UIMessage | undefined,
    status: ReturnType<typeof useChatIntegration>["status"],
    selectedCharacterId: string | null
): string => {
    const fromMeta = asMessageWithMetadata(message).metadata?.characterId
    if (fromMeta) return normalizeCharacterId(fromMeta)
    ...
    if (
        ...
        return normalizeCharacterId(selectedCharacterId)
    }

    return normalizeCharacterId(undefined)
}
...
export function Messages({
    ...
    const selectedCharacterId = useModelStore((state) => state.selectedCharacterId)
            ...
            <div className="min-h-[90dvh] overflow-y-auto p-4 pt-0" ref={scrollRef}>
                ...
                >
                    {messages.map((message) => {
                        const displayCharacterId = getMessageCharacterIdForAvatar(
                            message,
                            lastMessage,
                            status,
                            selectedCharacterId
                        ...
                        const bubbleClassName = cn(
                            ...
                            message.role === "user" &&
                                ...
                                "my-12 w-fit max-w-md rounded-md border border-border bg-secondary/50 px-4 py-2 text-foreground"
                        )
                        ...
                        return (
                            <div
                                key={message.id}
                                className={cn(
                                    "mb-8 flex flex-nowrap items-end gap-2",
                    ...
                    {status === "error" && (
                        <div
                            role="alert"
                            aria-live="assertive"
                            className="flex items-start gap-3 rounded-lg border border-destructive/35 bg-destructive/10 p-4 text-foreground shadow-sm"
                        >
                            <AlertCircle
                            ...
                            <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                {lastUserMessage ? (
                                    <Button
                                        ...
                                        aria-label="Retry last message after error"
                                    >
